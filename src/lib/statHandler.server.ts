import { R6AUTH } from '$env/static/private';
...
async function getAuth() {
	...
	const ticket: string = obj.ticket;
	const expire: string = obj.expiration;
	return { sessionId, ticket, expire };
}

export async function getPlayerIdByUsername(username: string, platform: string) {
	const { ticket } = await getAuth();
